...
## 更新摘要
**变更内容**
- 更新了ComponentInterface接口的getLayout()方法规范，现在要求返回包含elements和components的统一数组格式
- 新增了components数组的详细说明，支持v-if动态子组件声明
- 更新了Application类中对components数组的处理逻辑
- 修改了生成组件的实现示例，展示新的getLayout()返回格式
...
**更新** 本版本重点反映了组件接口标准化的最新变化，getLayout()方法现在要求返回统一的elements和components数组格式。
...
**图表来源**
- [ComponentInterface.php:13-65](file://framework/interfaces/ComponentInterface.php#L13-L65)
...
- [sfc-compiler.php:520-577](file://framework/sfc-compiler.php#L520-L577)
- [AppComponent.php:11-811](file://apps/calculator/gen/AppComponent.php#L11-L811)

**章节来源**
- [ComponentInterface.php:1-65](file://framework/interfaces/ComponentInterface.php#L1-L65)
...
- [sfc-compiler.php:520-577](file://framework/sfc-compiler.php#L520-L577)
- [AppComponent.php:1-811](file://apps/calculator/gen/AppComponent.php#L1-L811)

## 核心组件
- ComponentInterface：定义组件必须实现的核心契约，包括标识、布局、父子关系、属性与生命周期回调。**更新** getLayout()方法现在要求返回包含elements和components的统一数组格式。
- BaseComponent：实现 ComponentInterface，提供组件树基础能力（父子引用、子组件集合、属性、挂载状态与后代遍历）。**更新** getLayout()方法为抽象方法，要求子类实现。
- ReactiveComponent：继承 BaseComponent，增加响应式状态与脏标记机制，作为业务组件的基类。
- Application：应用入口，负责窗口初始化、组件挂载/卸载、布局收集与渲染调度。**更新** 新增对components数组的动态组件管理支持。
- 生成组件：由 SFC 编译器根据模板生成，实现 getLayout()/onAttach()/onDetach() 等方法。**更新** getLayout()方法返回统一的elements和components格式。
...
## 架构总览
组件接口在组件系统中的作用：
- 统一契约：保证所有组件具备一致的标识、布局、父子关系与生命周期行为。**更新** getLayout()方法返回格式标准化。
- 生命周期管理：onAttach/onDetach 与 Application 的挂载/卸载流程配合，确保资源正确初始化与释放。
- 布局数据获取：getLayout 返回标准化的 elements/components 结构，便于应用层统一收集与偏移应用。**更新** components数组支持v-if动态子组件声明。
...
App->>IF : 调用 getLayout()
IF-->>App : 返回 ['elements' => [...], 'components' => [...]]
App->>App : 收集布局并应用累积偏移
App->>App : 处理 components 数组进行动态挂载
...
**图表来源**
- [Application.php:82-103](file://framework/Application.php#L82-L103)
- [Application.php:143-200](file://framework/Application.php#L143-L200)
...
- getLayout(): array
  - 功能：返回组件布局数据，格式为 ['elements' => [...], 'components' => [...]]。elements包含rect/text/button等几何元素；components包含v-if动态子组件声明。
  - 参数：无
  - 返回：标准布局数组，包含elements和components两个键
  - 使用场景：Application 收集布局、应用累积偏移、渲染器绘制、动态组件管理
  - 参考实现：各生成组件的 getLayout()
  - **更新** 现在要求返回统一的elements和components格式，支持v-if动态子组件声明
...
**章节来源**
- [ComponentInterface.php:18-64](file://framework/interfaces/ComponentInterface.php#L18-L64)
...
- **更新** getLayout()方法现在为抽象方法，要求子类必须实现统一的返回格式。
...
- 布局收集：collectLayoutRecursive 递归遍历组件树，应用累积偏移（x/y），合并 elements/components。**更新** 新增对components数组的处理，支持v-if动态子组件的条件挂载/卸载。
...
Collect --> ProcessElements["处理 elements 数组<br/>应用累积偏移"]
ProcessElements --> ProcessComponents["处理 components 数组<br/>动态挂载/卸载"]
ProcessComponents --> Merge["合并 elements/components"]
Merge --> End(["结束"])
```

**图表来源**
- [Application.php:82-91](file://framework/Application.php#L82-L91)
- [Application.php:143-200](file://framework/Application.php#L143-L200)
- [AppComponent.php:684-701](file://apps/calculator/gen/AppComponent.php#L684-L701)
...
### 生成组件示例与实现模式
- AppComponent：根组件，定义业务状态与方法，生成 registerChildren() 注册子组件，实现 getLayout() 返回自身布局与按钮，以及 getBaseComponents() 供 Application 初始化。**更新** getLayout()方法现在返回统一的elements和components格式。
...
**章节来源**
- [AppComponent.php:190-389](file://apps/calculator/gen/AppComponent.php#L190-L389)
- [DisplayPanelComponent.php:15-83](file://apps/calculator/gen/DisplayPanelComponent.php#L15-L83)
- [NumPadComponent.php:15-214](file://apps/calculator/gen/NumPadComponent.php#L15-L214)
- [AboutDialogComponent.php:15-214](file://apps/calculator/gen/AboutDialogComponent.php#L15-L214)
...
- Application 依赖 ComponentInterface 进行组件树遍历、挂载/卸载与布局收集。**更新** 新增对components数组的动态组件管理支持。
...
- **更新** 动态组件管理：Application 对components数组的处理采用条件判断和状态追踪，避免重复挂载/卸载，提高性能。
...
- [Application.php:269-311](file://framework/Application.php#L269-L311)
...
- **更新** getLayout()返回格式错误：确保getLayout()方法返回包含elements和components键的数组，且components数组格式正确。
...
## 结论
ComponentInterface 为组件系统提供了统一的契约与能力边界，结合 BaseComponent/ReactiveComponent 与 Application 的协作，实现了清晰的组件树管理、生命周期控制与高效渲染。**更新** 最新版本标准化了getLayout()方法的返回格式，统一了elements和components数组结构，新增了对v-if动态子组件的支持。遵循 AOT 兼容性最佳实践，可确保在静态编译环境下稳定运行。通过生成组件的实现模式，开发者可以快速扩展新的业务组件并融入现有体系。

## 附录
- 自定义组件实现步骤
  1) 新建类并继承 ReactiveComponent（或 BaseComponent，如无需响应式能力）
  2) 实现 getLayout() 返回包含elements和components的统一数组格式
  3) 实现 onAttach()/onDetach() 完成资源初始化与释放
  4) 如为根组件，实现 getBaseComponents() 或让编译器生成
  5) 在 Application 初始化时挂载组件树
- **更新** getLayout()方法返回格式规范
  - elements: 包含rect/text/button等几何元素的数组
  - components: 包含v-if动态子组件声明的数组，格式为['type' => 'ComponentClassName', 'key' => 'uniqueKey', 'props' => [...], 'vIf' => [...]]
- 参考实现路径
  ...
  - [生成组件示例:190-389](file://apps/calculator/gen/AppComponent.php#L190-L389)
  - [生成组件示例:15-83](file://apps/calculator/gen/DisplayPanelComponent.php#L15-L83)
  - [生成组件示例:15-214](file://apps/calculator/gen/NumPadComponent.php#L15-L214)
  - [生成组件示例:15-214](file://apps/calculator/gen/AboutDialogComponent.php#L15-L214)